...
/**********************************************************************************
CH395 TCP/IP 协议族接口
MSC51 演示程序，用于演示CH395的低能耗模式，GPIO操作，及EEPROM(4K)操作。MCS51@24MHZ,KEIL 3.51
**********************************************************************************/
/* 头文件包含*/
#include <reg52.h>
#include "stdio.h"
...
void CH395SocketInterrupt(UINT8 sockindex)
{
   ...
   /*
   **在TCP应用时产生断开中断和超时中断时，需要在程序上使用CH395CloseSocket()函数关闭此Socket，再次使用
   **就打开，需要连接就连接，如果程序上没有关闭此Socket会导致此Socket不会被释放，不能再使用。
   **
   */
...
UINT8  CH395Init(void)
{
	...
	/* CH395进入低能耗模式,屏蔽SINT_STAT_SEND_OK中断*/
...
void CH395GpioTest(void)
{
   ...
   CH395WriteGPIOAddr(GPIO_DIR_REG,0xff);                            /*设输出*/
   CH395WriteGPIOAddr(GPIO_PU_REG, 0xff);       					 /*上拉*/
...
int main(void)
{
	...
	printf("设置PHY连接方式：PHY_10M_FLL \n");
	CH395CMDSetPHY(PHY_10M_FLL);	                                  /*设置PHY连接方式  */
	mDelaymS(200);   
	mDelaymS(200);   
