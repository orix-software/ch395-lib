...
void CH395SocketInterrupt(UINT8 sockindex)
{
   ...
   if(sock_int_socket & SINT_STAT_RECV)                              /* 接收中断 */
   {
       ...
       tmp = ((UINT16)MyBuffer[1] << 8) + MyBuffer[0];
       port = ((UINT16)MyBuffer[3] << 8) + MyBuffer[2];
#if CH395_DEBUG
       printf("Data Len = %d\n",tmp);
       printf("Port = %d\n",port);
       printf("IP = %2d.%2d.%2d.%2d.\n",(UINT16)MyBuffer[4],(UINT16)MyBuffer[5],(UINT16)MyBuffer[6],(UINT16)MyBuffer[7]);      
#endif
       CH395UDPSendTo(&MyBuffer[8], (len -8),&MyBuffer[4], port, sockindex);
       return;       
